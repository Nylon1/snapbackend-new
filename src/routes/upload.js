...
const getMediaCMSToken = require("./mediacmsToken"); // <-- import helper
...
router.post("/", upload.single("video"), async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // If token expired/invalid, force refresh and try again
      if (err.response && (err.response.status === 401 || err.response.status === 403)) {
        token = await getMediaCMSToken(true); // force refresh
        mcmsRes = await axios.post(
          "https://mediacms-cw-u46015.vm.elestio.app/api/v1/media/",
          form,
          {
